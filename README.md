...
npm test         # 15 unit test domain + storage (vitest)
...
```js
// 1. Read failure — alert page-level, dipicu dari jalur boot()
...
Keempat error path itu juga ditutup test: validasi di `tests/domain.test.ts`, read-fail dan
write-fail di `tests/storage.test.ts`.
