...
const e = (over: Partial<Entry> = {}): Entry => ({
  id: 'a', name: 'Kopi Toko', city: 'Bandung', drink: 'Latte',
  rating: 5, notes: 'enak', createdAt: NOW, ...over,
});

describe('normalizeDraft', () => {
  it('trims and collapses whitespace, coerces rating', () => {
...
describe('validateDraft', () => {
  it('accepts a complete draft', () => {
    expect(validateDraft(normalizeDraft(e()))).toEqual({});
  });
  it('flags every empty required field', () => {
    expect(Object.keys(validateDraft(normalizeDraft({}))).sort())
      .toEqual(['city', 'drink', 'name', 'notes', 'rating']);
  });
  it('rejects ratings outside 1-5 and whitespace-only notes', () => {
...
describe('normalizeEntry — the single gate for form and storage', () => {
  it('rejects non-objects and corrupt rows', () => {
    for (const bad of [null, 'x', 7, [], { name: 'Kopi' }, { ...e(), rating: 9 }]) {
      expect(normalizeEntry(bad, NOW, 'fb')).toBeNull();
    }
  });
  it('keeps a stored id and createdAt, repairs missing ones, and normalizes', () => {
...
describe('filterEntries', () => {
  ...
  it('returns a copy for "all" and only 4 stars and up for "high"', () => {
...
describe('sortEntries', () => {
  ...
  it('sorts newest first by default', () => {
    expect(sortEntries(list, 'recent').map((x) => x.id)).toEqual(['c', 'b', 'a']);
  });
  it('sorts city A-Z ignoring case', () => {
    expect(sortEntries(list, 'city').map((x) => x.city)).toEqual(['Ambon', 'bandung', 'Cirebon']);
  });
  it('sorts rating high to low, newest first on ties, without mutating', () => {
